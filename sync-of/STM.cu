...
__device__ TX_Data* TX_Init(STMData* stm_data, int tx_id, int* locator_queue)
{
  ...
  { 
    int lock = stm_data -> lock;
    assert(atomicCAS(&stm_data-> lock,lock,gridDim.x)==lock); 
  } 
...
__device__ void sync_all_threads(STMData* stm_data){ 
  __syncthreads(); 
  int lock;
  if(threadIdx.x == 0) 
  { 
    do{ 
      lock = stm_data -> lock; 
    } while (atomicCAS(&stm_data-> lock,lock,lock-1)!=lock); 
  }
 
 while(lock>0);
 
 if (threadIdx.x == 0 && blockIdx.x == 0) 
  { assert(atomicCAS(&stm_data-> lock,0,gridDim.x)==0); } 
